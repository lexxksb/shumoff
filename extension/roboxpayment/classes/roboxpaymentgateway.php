...
class roboxpaymentGateway extends eZRedirectGateway {

    const GATEWAY_NAME = "roboxpayment";

    function createPaymentObject($processID, $orderID) {
        //__DEBUG__
        $this->logger->writeTimedString("createPaymentObject");
        return eZPaymentObject::createNew($processID, $orderID, self::GATEWAY_NAME);
        //___end____
        return null;
    }
    ...
    function createRedirectionUrl($process) {
        ...
        // get order items
        $orderItems = eZOrderItem::fetchList( $orderID );
        if(isset($orderItems[0]->Price)) $shipPrice = $orderItems[0]->Price;
        else $shipPrice = 0;
        ...
        if ($order instanceof eZOrder) {
            ...
            if ($xmlString != null) {
                ...
                $accountName = $firstNameNode->textContent . ' ' . $lastNameNode->textContent . " " . $emailNode->textContent;
            }

            $MrchLogin = $ini->variable('PaySettings', 'MrchLogin');
            $OutSum = $order->productTotalIncVAT() + $shipPrice;
            $InvId = $order->ID;
            $SignatureValue = md5($MrchLogin.":".$OutSum.":".$InvId.":".$ini->variable('PaySettings', 'pass1'));
            
            $params["MrchLogin"] = $MrchLogin;
            $params["OutSum"] = $OutSum;
            $params["InvId"] = $InvId;
            $params["Desc"] = $accountName;
            $params["SignatureValue"] = $SignatureValue;
            //$params["IncCurrLabel"] = "rub";
            $params["Culture"] = "ru";

            $url = $ini->variable('PaySettings', 'server')."?".http_build_query($params);

            $this->logger->writeTimedString("[URL] ".$url);
...
// define('ROBOXPAYMENT_BASE', 'roboxpayment');
// define('ROBOXPAYMENT_STATE_CANCEL', 2);
// define('ROBOXPAYMENT_STATE_VALID', 5);

// class roboxpaymentGateway extends eZPaymentGateway {
//   const GATEWAY_NAME = "roboxpayment";

//   /*!
//    Constructor.
//    */
//   function roboxpaymentPaymentGateway() {
//     //$this->logger = eZPaymentLogger::CreateForAdd("var/log/roboxpaymentGateway.log");
//     //$this->logger->writeTimedString('roboxpaymentGateway::roboxpaymentGateway');
//   }

//   function execute($process, $event) {
//     $this->logger->writeTimedString("execute RoboxPay");
//     $this->logger->writeTimedString("[RoboxPay] event_state:" . $process->attribute('event_state'));
...
//     //$this->fetchInput($http, ROBOXPAYMENT_BASE, $event, $process);

//     $this->logger->writeTimedString("[RoboxPay] event_state:" . $process->attribute('event_state'));

//     if ($process->attribute('event_state') == ROBOXPAYMENT_STATE_CANCEL) {
//       return eZWorkflowType::STATUS_WORKFLOW_CANCELLED;
//     }

// //     if ($process->attribute('event_state') != ROBOXPAYMENT_STATE_VALID) {
// //       //$orderLogArray = eZOrderLog::fetchByEmail($accountInfo['email']);

// //       $process->Template = array();
// //       $process->Template['templateName'] = 'design:roboxpayment/payment-form.tpl';
// //       $process->Template['templateVars'] = array(
// //           'process' => $process, 
// //           'event' => $event, 
// //           'base' => ROBOXPAYMENT_BASE, 
...
//           $ini = eZINI::instance('roboxpayment.ini');
...
eZPaymentGatewayType::registerGateway(roboxpaymentGateway::GATEWAY_NAME, "roboxpaymentGateway", "Оплата робокассой");
