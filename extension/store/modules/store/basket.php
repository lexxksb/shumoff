...
if ( $http->hasPostVariable( "StoreChangesButton" ) )
{
    ...
    $module->redirectTo( '/store/updatebasket/' );
    return;
}
...
// форма регистрации
include_once 'extension/store/modules/userform.php';
// форма регистрации
...
if ( $http->hasPostVariable( "CheckoutButton" ) or ( $doCheckout === true ) )
{
    ...
    {
        if(!$http->hasSessionVariable("input_error")){
            ...
            if ( $verifyResult === true )
            {
                ...
                $db->commit();
                $module->redirectTo( '/store/confirmorder/' );
                return;
            }
            else
            {
...
$Result = array();
$Result['content'] = $tpl->fetch( "design:store/basket.tpl" );
